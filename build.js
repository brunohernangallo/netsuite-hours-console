...
const VIEWS = {
  ...
  help() {
    return '<h1 class="page">Help</h1><p class="lede">What each column means and where the number comes from. Every figure here is read from NetSuite; none of it is estimated and none of it is derived from money.</p>'+
      GLOSSARY +
      '<div class="note"><b>The exact NetSuite field behind every number on this site.</b> Nothing here is estimated, and no hours figure is ever derived from money.'+
      '<table style="margin-top:8px;font-size:12.5px"><thead><tr><th>What you see</th><th>NetSuite record</th><th>Field</th><th>How it is read</th></tr></thead><tbody>'+
      [['Sold','opportunity','item lines 1000.xx / 2000.xx / 1000PS, quantity','Reached from the project through <code>custentity_bpc_primaryopportunity</code>. Duplicated lines are collapsed. Blank when the won opportunity carries no line items.'],
       ['CO','opportunity','same item lines','Won opportunities whose <code>custbody_bpc_opp_original_sow</code> points at the project. A CO that repeats the original SOW line items is a restatement and is not added.'],
       ['Plan','projecttask','<code>plannedwork</code>','Root tasks only (<code>parent IS NULL</code>), minus the Non-Billable node. A project task list is a tree - summing every level double counts.'],
       ['Allocated','job','<code>allocatedtime</code>','Identical to the sum of <code>resourceallocation.numberhours</code>. This is the PMO / EAC view: forward staffing, not time spent.'],
       ['Used','projecttask','<code>actualwork</code>','Root tasks, minus the Non-Billable node. Billable only.'],
       ['Non-billable','projecttask','<code>actualwork</code> on the Non-Billable node','Displayed on its own. Never added to Used, Remaining or any total on this site.'],
       ['Billed / Unbilled','charge','<code>stage</code> = BILLED / READY_FOR_BILLING','Joined to the project through <code>timebill.customer</code>, restricted to <code>use = Actual</code>.'],
       ['On invoices','transaction + transactionline','<code>type = CustInvc</code>, <code>transactionline.entity</code> = the project','Hours per invoice come from the charges attached to it. The project is on the invoice LINE, not the header - the header entity is the parent customer.'],
       ['Invoiced / Unpaid','transaction','<code>foreignamount</code> on the lines, <code>foreignamountunpaid</code> on the header','If one invoice spans two projects the unpaid figure is the whole invoice.'],
       ['Last time booked / 30d / people','timebill','<code>trandate</code>, <code>hours</code>, <code>employee</code>','Where the time is landing lists the top tasks of the last 60 days.'],
       ['Renewal, term, rollover','job','<code>startdate</code>, <code>enddate</code>, <code>custentity_bpc_ms_renewal_date</code>, rollover cap','Managed Services only.'],
       ['Links','job','<code>custentity_bpc_rtmproject</code>, <code>custentity_bpc_sow_link</code>','RTM project and the signed SOW in Drive.']]
      .map(r=>'<tr><td><b>'+r[0]+'</b></td><td class="muted">'+r[1]+'</td><td><code>'+r[2]+'</code></td><td>'+r[3]+'</td></tr>').join('')+
      '</tbody></table></div>'+
      '<div class="note"><b>Fields we deliberately do NOT use.</b> <code>custentity_bpc_esthours</code> is empty on every project. <code>projecttask.estimatedwork</code> reads the whole tree and gives wrong totals. Money divided by a rate is never used to produce hours - the rate on a project is not constant, so the answer would be invented. And the per-month retainer block (<code>custevent_bpc_prepaid_hours</code>) only exists on the monthly tasks, so it can never be compared against a whole project plan.</div>'+
      '<div class="note"><b>How the table works.</b> Click a column header to sort by it, click again to reverse; it opens sorted alphabetically by client. The filter box at the top narrows every view at once. A client or SOW name opens the full detail: hours, change orders, invoicing, closed SOWs and where the time is landing. The References links at the end of each row go to the NetSuite project, the opportunity, the RTM and the signed SOW so you can check any number yourself.</div>'+
      '<div class="note"><b>Colours.</b> Green is under 85% of what was bought, amber 85% or more, red past it. Non-billable time is grey and italic, and is never added to anything.</div>';
  },
  quality(q) {
    const rs = D.rows.filter(r=>match(r,q));
    const g = k => rs.filter(r=>r.issues.includes(k));
